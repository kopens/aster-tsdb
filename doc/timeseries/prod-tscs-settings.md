...
> **정정 (2026-09-13 실측): `tm_tag_point`도 이제 파킹이 있습니다 — 다만 A/B그룹과는 다른
> 이유입니다.** 노드 10.99.0.101에서 창 2개(2026-09-08, 09-09)가 파킹됐고, 넘긴 파티션은 태그
> `TAG_LS_ARR100`(파티션 키 `tag_id`) 하나이며 그 sstable이 `[09-04..09-08]`을 걸칩니다. 같은 시점
> `tm_tag_point_snapshot`은 양 노드에서 창 8개(09-02~09-09)로 §2.0의 기존 진단과 일치합니다.
>
> **A/B그룹의 "무한히 자라는 파티션"이 아닙니다.** 이 테이블은 티어링(hot 12h)이 켜져 있어 base
> 파티션이 12시간치로 유계입니다. 그런데도 예산을 넘기는 것은 **이 태그 한 개가 폭이 넓고
> 조밀하기 때문**입니다:
>
> | 항목 | 실측 |
> | --- | --- |
> | `value` 길이 | **1,501 B** (같은 테이블 `TAG_LS_FLOAT`는 6 B) |
> | 쓰기 주기 | 초당 1행 급 |
> | 청크 실측 | 15분 창당 621샘플 → ≈0.93 MB/15분 → **하루 ≈89 MB** |
> | `tablestats` compacted partition max (10.99.0.101) | **89.97 MB** — 위 계산과 일치 |
>
> `gc_grace_seconds=86400`이라 하루짜리 창 안에 *살아 있는 12h + 아직 못 지운 12h*가 함께 들어가고,
> 그 합이 64 MiB 예산을 넘습니다.
>
> **따라서 처방도 A/B그룹과 다릅니다.** 월·일 단위 재파티셔닝은 **효과가 없습니다** — 12시간치가
> 이미 예산을 채우므로 시간 단위 버킷이라야 유효한데 그건 과합니다. `window_size` 확대도 여기서는
> 역효과입니다(창이 커질수록 창당 바이트가 늘어난다 — snapshot에 권한 확대는 경계 걸침 *빈도*를
> 줄이는 다른 목적이다). 값싼 후보 순서:
>
> 1. **`hot_window` 축소 (12h → 6h)** — 티어링 확장 ALTER 한 줄. 창당 라이브 바이트가 절반이 되고,
>    `chunk_window` 15m 대비 24배 여유라 [production-rollout.md §6](production-rollout.md)의
>    "hot_window와 chunk_window를 같게 두지 마라" 조건에도 걸리지 않는다. 대가는 6~12h 구간 질의가
>    병합 비용을 물게 되는 것.
> 2. **그 태그를 별도 테이블로 분리** — 1.5 KB 배열 값이 초당 들어오는 태그가 스칼라 태그 396개와
>    한 테이블에 있는 것이 맞는지 자체가 질문이다.
>
> 급하지는 않습니다(현재 창 2개). 다만 `tm_tag_point`는 `retention=3651d`(≈10년)이라 **§2.1이
> `tm_tag_point_snapshot`에 적용한 "파킹 수명이 retention으로 유계"라는 위안이 여기엔 없습니다** —
> 방치하면 사실상 영구 잔존이고 시간이 갈수록 누적됩니다. 계층화 자체는 이 파킹에 막히지 않습니다
