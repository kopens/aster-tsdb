...
|---|---|---|---|
| single partition, aggregates over value_numeric (tag-000001, type='double', 40,000 rows scanned) | count(*) [40,000 rows] | `40000` | **50 ms** |
| single partition, aggregates over value_numeric (tag-000001, type='double', 40,000 rows scanned) | time_bucket 1h + avg/min/max(value_numeric) [40,000 rows] | `2024-01-01 11:00:00 \| 14.3501 \| 14.18 \| 14.44` | **56 ms** |
| single partition, aggregates over value_numeric (tag-000001, type='double', 40,000 rows scanned) | time_bucket 5m + avg(value_numeric) [40,000 rows] | `2024-01-01 11:05:00 \| 14.3752` | **52 ms** |
| single partition, aggregates over value_numeric (tag-000001, type='double', 40,000 rows scanned) | first/last/delta/rate(value_numeric) per hour [40,000 rows] | `2024-01-01 11:00:00 \| 14.19 \| 14.34 \| 0.15 \| 0.00037594` | **64 ms** |
| single partition, aggregates over value_numeric (tag-000001, type='double', 40,000 rows scanned) | derivative(value_numeric) per hour [40,000 rows] | `2024-01-01 11:00:00 \| 0.0003569` | **51 ms** |
| single partition, aggregates over value_numeric (tag-000001, type='double', 40,000 rows scanned) | percentile p50/p95/p99(value_numeric) [40,000 rows] | `13.38 \| 14.12 \| 14.28` | **41 ms** |
| single partition, aggregates over value_numeric (tag-000001, type='double', 40,000 rows scanned) | variance/stddev(value_numeric) [40,000 rows] | `0.149982 \| 0.387275` | **31 ms** |
| single partition, aggregates over value_numeric (tag-000001, type='double', 40,000 rows scanned) | histogram(value_numeric, 0, 100, 20) [40,000 rows] | `[0, 0, 0, 40000, 0, 0, 0,~` | **34 ms** |
| single partition, aggregates over value_numeric (tag-000001, type='double', 40,000 rows scanned) | approx_count_distinct(value_numeric) [40,000 rows] | `186` | **38 ms** |
| single partition, aggregates over value_numeric (tag-000001, type='double', 40,000 rows scanned) | integral + time_weighted_average(value_numeric) [40,000 rows] | `537779 \| 13.4448` | **43 ms** |
| single partition, aggregates over value_numeric (tag-000001, type='double', 40,000 rows scanned) | SECONDARY: time_bucket 1h + avg/min/max(latency) [40,000 rows] | `2024-01-01 11:00:00 \| 501 \| 5 \| 998` | **59 ms** |
| row reads (all 16 columns vs. projections) -- type-independent | newest 1000 rows, SELECT * -- type='boolean' tag (tag-000000) | `tag-000000 \| 2024-01-01 11:06:39 \| area-a \| asset-0000 \| line-01 \` | **45 ms** |
| row reads (all 16 columns vs. projections) -- type-independent | newest 1000 rows, SELECT * -- type='double' tag (tag-000001) | `tag-000001 \| 2024-01-01 11:06:39 \| area-b \| asset-0001 \| line-02 \` | **38 ms** |
| row reads (all 16 columns vs. projections) -- type-independent | 1 hour of rows, SELECT * (all columns) [3,600 rows] | `tag-000001 \| 2024-01-01 00:59:59 \| area-b \| asset-0001 \| line-02 \` | **138 ms** |
| row reads (all 16 columns vs. projections) -- type-independent | 1 hour of rows, project timestamp+value only [3,600 rows] | `2024-01-01 00:59:59 \| 13.58` | **33 ms** |
| row reads (all 16 columns vs. projections) -- type-independent | 1 hour of rows, project timestamp+value_numeric only [3,600 rows] | `2024-01-01 00:59:59 \| 13.58` | **30 ms** |
...
| row reads (all 16 columns vs. projections) -- type-independent | column presence, type='boolean' tag [40,000 rows] | `40000 \| 40000 \| 0 \| 40000 \| 40000 \| 40000` | **65 ms** |
| row reads (all 16 columns vs. projections) -- type-independent | column presence, type='double' tag [40,000 rows] | `40000 \| 40000 \| 40000 \| 0 \| 40000 \| 40000` | **60 ms** |
| gap-fill (locf/interpolate over avg(value_numeric), numeric tag tag-000001) | gapfill 1h + locf over the full 11h span [40,000 rows] | `2024-01-01 00:00:00 \| 13.63` | **53 ms** |
| gap-fill (locf/interpolate over avg(value_numeric), numeric tag tag-000001) | gapfill 5m + interpolate over 6h [21,600 rows] | `2024-01-01 00:00:00 \| 13.7812` | **30 ms** |
| multi-partition (numeric-typed tags only -- 90 of 500 tags are numeric) | 10 numeric tags, hourly avg(value_numeric) [400,000 rows] | `tag-000001 \| 2024-01-01 11:00:00 \| 14.3501` | **478 ms** |
| multi-partition (numeric-typed tags only -- 90 of 500 tags are numeric) | 90 numeric tags, hourly avg(value_numeric) [3,600,000 rows] | `tag-000001 \| 2024-01-01 11:00:00 \| 14.3501` | **4,216 ms** |
| multi-partition (numeric-typed tags only -- 90 of 500 tags are numeric) | 90 numeric tags, p95(value_numeric) per tag [3,600,000 rows] | `tag-000001 \| 14.12` | **2,619 ms** |
| dashboard query (numeric tag tag-000001) | OHLC + change + p95 of value_numeric per hour [40,000 rows] | `2024-01-01 11:00:00 \| 400 \| 14.19 \| 14.34 \| 14.18 \| 14.44 \| 14.3` | **82 ms** |
| full table scan (20,000,000 rows) -- WRONG ANSWER on a tiered table (known limitation) | count(*) over the whole table | `500` | **16 ms** |

## Details

### single partition, aggregates over value_numeric (tag-000001, type='double', 40,000 rows scanned)

**count(*) [40,000 rows]** — `50 ms`
...
**time_bucket 1h + avg/min/max(value_numeric) [40,000 rows]** — `56 ms`
...
**time_bucket 5m + avg(value_numeric) [40,000 rows]** — `52 ms`
...
**first/last/delta/rate(value_numeric) per hour [40,000 rows]** — `64 ms`
...
**derivative(value_numeric) per hour [40,000 rows]** — `51 ms`
...
**percentile p50/p95/p99(value_numeric) [40,000 rows]** — `41 ms`
...
**variance/stddev(value_numeric) [40,000 rows]** — `31 ms`
...
**histogram(value_numeric, 0, 100, 20) [40,000 rows]** — `34 ms`
...
**approx_count_distinct(value_numeric) [40,000 rows]** — `38 ms`
...
**integral + time_weighted_average(value_numeric) [40,000 rows]** — `43 ms`
...
**SECONDARY: time_bucket 1h + avg/min/max(latency) [40,000 rows]** — `59 ms`
...
**newest 1000 rows, SELECT * -- type='double' tag (tag-000001)** — `38 ms`
...
**1 hour of rows, SELECT * (all columns) [3,600 rows]** — `138 ms`
...
**1 hour of rows, project timestamp+value only [3,600 rows]** — `33 ms`
...
**1 hour of rows, project timestamp+value_numeric only [3,600 rows]** — `30 ms`
...
**column presence, type='boolean' tag [40,000 rows]** — `65 ms`
...
**column presence, type='double' tag [40,000 rows]** — `60 ms`
...
**gapfill 1h + locf over the full 11h span [40,000 rows]** — `53 ms`
...
**gapfill 5m + interpolate over 6h [21,600 rows]** — `30 ms`
...
**10 numeric tags, hourly avg(value_numeric) [400,000 rows]** — `478 ms`
...
**90 numeric tags, hourly avg(value_numeric) [3,600,000 rows]** — `4,216 ms`
...
**90 numeric tags, p95(value_numeric) per tag [3,600,000 rows]** — `2,619 ms`
...
**OHLC + change + p95 of value_numeric per hour [40,000 rows]** — `82 ms`
...
**count(*) over the whole table** — `16 ms`
