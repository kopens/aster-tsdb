...
[CHANGELOG.md](CHANGELOG.md) is generated by CI from a `Changelog:` git trailer, so **a commit
without the trailer never appears in it**. That is deliberate: every merge with
`apache/cassandra` brings thousands of upstream commits, and the trailer is what separates this
fork's changes from them. Upstream commits must therefore stay untrailered.

Add the trailer as its own last paragraph of the commit message, and only for changes a *user or
operator* of this build would notice — a new function, a behaviour change, a fix, a dependency bump
that affects deployment. Refactors, test-only changes and documentation do not get one.

```
Tiering: DROP TABLE refuses to orphan or destroy the chunk table

<the body explaining why>

Changelog: fixed
```

Valid values, matching `.gitlab/changelog_config.yml`: `added`, `changed`, `deprecated`, `removed`,
`fixed`, `security`, `performance`, `other`. A value outside that list is dropped with a warning in
the job log rather than failing the build, so it goes missing quietly — check the spelling.

The commit subject becomes the entry text, so write the subject to stand alone in a release note.

There is no need to edit CHANGELOG.md by hand for a release; the `changelog` CI job writes the
section when a tag is pushed. `CHANGES.txt` is a separate, upstream-facing file and keeps its own
Apache convention — see the note at the top of CHANGELOG.md.
